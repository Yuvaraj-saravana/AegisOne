...
function App() {
  ...
  const filteredTickets = tickets.filter(t => t.title.toLowerCase().includes(ticketSearchQuery.toLowerCase()) || (t.ticket_id && t.ticket_id.toLowerCase().includes(ticketSearchQuery.toLowerCase())));
                        ...
                        <tbody className={`divide-y ${curTheme.border}`}>
                          {filteredTickets.map(ticket => (
                            <tr key={ticket.ticket_id} className="hover:bg-black/5 transition-colors cursor-pointer" onClick={() => setSelectedTicket(ticket)}>
                              <td className="px-6 py-4"><div className="flex flex-col"><span className={`font-medium ${curTheme.heading} mb-1`}>{ticket.title}</span><div className="flex gap-2 items-center"><span className={`font-mono text-[10px] ${curTheme.textMuted}`}>{ticket.ticket_id}</span><span className="text-[10px] text-slate-500">•</span><span className={`text-[10px] ${curTheme.textMuted}`}>{ticket.category}</span></div></div></td>
                          ...
                          {filteredTickets.length === 0 && <tr><td colSpan="6" className={`px-6 py-12 text-center ${curTheme.textMuted}`}>No tickets found in database.</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className={`relative w-full max-w-2xl h-full ${curTheme.surfaceSolid} border-l ${curTheme.border} shadow-2xl flex flex-col animate-in slide-in-from-right duration-300`}>
            <div className={`p-6 flex items-start justify-between border-b ${curTheme.border}`}>
              <div className="space-y-2">
                <div className="flex items-center gap-3"><span className={`font-mono text-sm ${curColor.text} font-bold`}>{selectedTicket.ticket_id}</span><span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border ${getTicketStatusBadge(selectedTicket.status)}`}>{selectedTicket.status}</span><span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border ${getSeverityBadge(selectedTicket.severity)}`}>{selectedTicket.severity}</span></div>
              ...
              <div className="grid grid-cols-2 gap-6 p-4 rounded-xl bg-black/5 border border-slate-800/20">
                ...
                <div>
                  ...
                  {selectedTicket.assigned_to ? (
                    <span className={`text-sm ${curTheme.heading}`}>{selectedTicket.assigned_to}</span>
                  ) : (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={handleAssignTicket} className={`text-xs px-2 py-1 bg-black/20 rounded border ${curTheme.border} hover:${curColor.text}`}>Assign</button>
                    </div>
                  )}
                </div>
              </div>
